...
export function DealForm({
  ...
  async function handleSave() {
    ...
    if (deal) {
      const { error } = await supabase
        ...
        .eq("id", deal.id);
      if (error) {
        toast.error(t("toast.failedSaveDeal"));
        setSaving(false);
        return;
      }
    } else {
    ...
    setSaving(false);
    toast.success(deal ? t("toast.dealUpdated") : t("toast.dealCreated"));
    onOpenChange(false);
    onSaved();
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("toast.failedDeleteDeal"));
      return;
    }
        ...
        <div className="flex h-full flex-col">
          <SheetHeader className="border-b border-slate-700/50 p-4">
            <SheetTitle className="text-white">
              {deal ? t("deal.editDeal") : t("deal.newDeal")}
            </SheetTitle>
          </SheetHeader>

          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="grid gap-2">
              <Label className="text-slate-300">{t("deal.title")}</Label>
              <Input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder={t("deal.dealTitle")}
                className="border-slate-700 bg-slate-800 text-white"
              />
            </div>

            <div className="grid gap-2">
              <Label className="text-slate-300">{t("deal.contact")}</Label>
              <select
                value={contactId}
            ...
            <div className="grid gap-2">
              <Label className="text-slate-300">{t("deal.expectedClose")}</Label>
              <Input
                type="date"
            ...
            <div className="grid gap-2">
              <Label className="text-slate-300">{t("deal.stage")}</Label>
              <select
                value={stageId}
            ...
            <div className="grid gap-2">
              <Label className="text-slate-300">{t("deal.assignedTo")}</Label>
              <select
                value={assignedTo}
            ...
            <div className="grid gap-2">
              <Label className="text-slate-300">{t("deal.notes")}</Label>
              <Textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder={t("deal.addNotes")}
          ...
          <div className="border-t border-slate-700/50 bg-slate-900/80 p-4">
            <div className="flex gap-2">
              ...
              >
                {saving ? t("deal.saving") : deal ? t("deal.saveChanges") : t("deal.createDeal")}
              </Button>
            </div>
              ...
              (confirmDelete ? (
                <div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                  ...
                  <div className="flex gap-1">
                    ...
                    >
                      {deleting ? t("deal.deleting") : t("deal.confirmDelete")}
                    </button>
                  </div>
                </div>
              ) : (
