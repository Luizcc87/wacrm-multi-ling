...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    onPipelinesChanged();
    onStagesChanged();
    toast.success(t("settings.pipelineSaved"));
  }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t("settings.failedAddStage"));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("settings.moveDealsFirst"));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t("settings.failedDeleteStage"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("settings.failedDeletePipeline"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t("settings.pipelineDeleted"));
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-slate-900 border-slate-700 max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-white">{t("settings.managePipeline")}</DialogTitle>
        </DialogHeader>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-400">
                  {t("settings.deletePipelineTitle")}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {t("settings.deletePipelineDesc")}
                </p>
              </div>
            </div>
          ...
          <>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label className="text-slate-300">{t("settings.pipelineName")}</Label>
                <Input
                  value={name}
              ...
              <div className="grid gap-2">
                <Label className="text-slate-300">{t("settings.stages")}</Label>
                <DndContext
                  sensors={sensors}
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={t("settings.dragReorder")}
      >
