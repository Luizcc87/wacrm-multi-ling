...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  useEffect(() => {
    async function fetchTemplates() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('template.failedLoad'));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-white">{t('steps.chooseTemplate')}</h2>
        <p className="mt-1 text-sm text-slate-400">
          {t('steps.chooseTemplateSubtitle')}
        </p>
      </div>
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-slate-800 bg-slate-900/50">
          ...
          <p className="text-sm text-slate-400">{t('template.noTemplates')}</p>
          <p className="mt-1 text-xs text-slate-500">{t('template.createFirst')}</p>
        </div>
      ) : (
