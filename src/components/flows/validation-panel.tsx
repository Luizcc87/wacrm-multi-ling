  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2 text-xs text-slate-400">
        ...
        {errors.length} {errors.length === 1 ? t("validation.errors") : t("validation.errorsPlural")},{" "}
        {warnings.length} {warnings.length === 1 ? t("validation.warnings") : t("validation.warningsPlural")}
      </div>
  ...
  if (issue.node_key && onJump) {
    return (
      <button
        ...
        aria-label={`${t("validation.jumpToNode")} ${issue.node_key}`}
      >
        {body}
      </button>
    );
