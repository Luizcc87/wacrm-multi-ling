...
export function AiKnowledgeCard({
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      ...
      else toast.error(data.error ?? t('toasts.loadFailed'));
    } catch {
  ...
  const openEdit = async (id: string) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.error ?? t('toasts.openFailed'));
        return;
      }
      setEditing(id);
      setTitle(data.title ?? '');
      ...
      toast.error(t('toasts.openFailed'));
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        else toast.success(isNew ? t('toasts.added') : t('toasts.updated'));
        cancelEdit();
        await fetchDocs();
      } else {
  ...
  const reindex = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        toast.success(t('toasts.reindexed', { count: data.reindexed }));
      } else {
        toast.error(data.error ?? t('toasts.reindexFailed'));
      }
    } catch {
      toast.error(t('toasts.reindexFailed'));
    } finally {
      setReindexing(false);
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t('description')}{' '}
          {hasEmbeddingsKey ? t('semanticOn') : t('semanticOff')}
        </CardDescription>
      </CardHeader>
            ...
            {editing !== null ? (
              <div className="space-y-3 rounded-md border border-border p-3">
                <div className="space-y-2">
                  <Label htmlFor="kb-title">{t('fields.title')}</Label>
                  <Input
                    id="kb-title"
                    value={title}
                ...
                <div className="space-y-2">
                  <Label htmlFor="kb-content">{t('fields.content')}</Label>
                  <Textarea
                    id="kb-content"
                    value={content}
