...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('api.toasts.loadFailed'));
        return;
      }
      ...
      toast.error(t('api.toasts.networkError'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('api.toasts.revokeFailed'));
        return;
      }
      toast.success(t('api.toasts.revokeSuccess', { name: key.name }));
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      <SettingsPanelHead
        title={t('api.title')}
        description={t.rich('api.description', {
          code: (chunks) => <code className="text-xs">{chunks}</code>,
          header: (chunks) => <code className="text-xs">{chunks}</code>,
        })}
        action={
          ...
          <CardContent className="flex flex-col items-center justify-center py-10 text-center">
            ...
            {canEditSettings ? (
              <p className="text-muted-foreground mt-1 text-xs">
                {t.rich('api.clickNewKey', {
                  btn: (chunks) => <span className="text-foreground">{chunks}</span>
                })}
              </p>
            ) : (
              <p className="text-muted-foreground mt-1 text-xs">
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground mt-1.5 text-xs">
                        {t('api.created', { date: fmtDate(k.created_at) })}
                        {' · '}
                        {k.last_used_at
                          ? t('api.lastUsed', { date: fmtDate(k.last_used_at) })
                          : t('api.neverUsed')}
                        {k.expires_at && status !== 'expired'
                          ? ` · ${t('api.expires', { date: fmtDate(k.expires_at) })}`
                          : ''}
                      </p>
                    </div>
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    if (!trimmed) {
      toast.error(t('api.toasts.nameRequired'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || t('api.toasts.createFailed'));
        return;
      }
  ...
  async function copyKey() {
    ...
    try {
      ...
      toast.success(t('api.toasts.copied'));
    } catch {
      toast.error(t('api.toasts.copyFailed'));
    }
  }
      ...
      <DialogContent className="border-border bg-popover sm:max-w-md">
        {createdKey ? (
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">
                {t('api.dialog.copyTitle')}
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('api.dialog.copyDesc')}
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-1.5">
              <Label className="text-muted-foreground">{t('api.dialog.labelKey')}</Label>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">
                {t('api.dialog.newTitle')}
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('api.dialog.newDesc')}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={name}
                  maxLength={80}
                  placeholder={t('api.dialog.namePlaceholder')}
                  onChange={(e) => setName(e.target.value)}
                />
              </div>

              <div className="space-y-2">
                <Label className="text-muted-foreground">{t('api.dialog.scopes')}</Label>
                ...
                <p className="text-muted-foreground text-xs">
                  {t.rich('api.dialog.noScopesHint', {
            ...
            <DialogFooter>
              ...
              <Button onClick={handleCreate} disabled={submitting}>
                {submitting ? (
                  <>
                    <Loader2 className="size-4 animate-spin" />
                    {t('api.dialog.creating')}
                  </>
                ) : (
                  t('api.dialog.createKey')
                )}
              </Button>
            </DialogFooter>
          </>
