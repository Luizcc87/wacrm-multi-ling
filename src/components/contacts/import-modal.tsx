...
export function ImportModal({ open, onOpenChange, onImported }: ImportModalProps) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (skipped > 0) {
        toast.info(`${skipped} ${t('duplicatesSkipped') || 'duplicados ignorados'}`);
      }
      if (failed > 0) {
        toast.error(`${failed} ${t('failed')}`);
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : t('importFailed') || 'Falha na importação';
      toast.error(message);
    } finally {
      setImporting(false);
    }
        ...
        <div className="space-y-4">
            ...
            {file ? (
              <>
                ...
                <p className="text-xs text-slate-500">
                  {parsedRows.length} {parsedRows.length !== 1 ? t('csvRowsDetected') : t('csvRowDetected')}
                </p>
              </>
            ) : (
              <>
          ...
          {preview.length > 0 && !result && (
            <div className="space-y-2">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">
                {t('previewLabel')} {preview.length} {t('previewRows')}
              </p>
              <div className="rounded-lg border border-slate-700 overflow-hidden">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-800">
                      <th className="px-3 py-1.5 text-left text-slate-400 font-medium">{t('previewPhone')}</th>
                      <th className="px-3 py-1.5 text-left text-slate-400 font-medium">{t('previewName')}</th>
                      <th className="px-3 py-1.5 text-left text-slate-400 font-medium">{t('previewEmail')}</th>
                      <th className="px-3 py-1.5 text-left text-slate-400 font-medium">{t('previewCompany')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.map((row, i) => (
              ...
              {parsedRows.length > 5 && (
                <p className="text-xs text-slate-500">
                  {t('importMore')} {parsedRows.length - 5} {t('importMoreRows')}
                </p>
              )}
            </div>
          )}
          ...
          {result && (
            <div className="rounded-lg border border-slate-700 p-4 space-y-2">
              <p className="text-sm font-medium text-white">{t('importComplete')}</p>
              <div className="flex flex-wrap items-center gap-4">
                ...
                {result.skipped > 0 && (
                  <div className="flex items-center gap-1.5 text-amber-400 text-sm">
                    ...
                    {result.skipped} {t('duplicatesSkipped') || 'duplicados ignorados'}
                  </div>
                )}
                {result.failed > 0 && (
        ...
        <DialogFooter className="bg-slate-900 border-slate-700">
          ...
          {!result && (
            ...
            >
              ...
              {t('importButton')} {parsedRows.length > 0 ? `${parsedRows.length} ${t('importContacts2')}` : ''}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
