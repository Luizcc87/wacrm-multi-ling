  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t('customFields.title')}</DialogTitle>
          <DialogDescription className="text-muted-foreground">{t('customFields.description')}</DialogDescription>
        </DialogHeader>
        <CustomFieldsPanel />
      </DialogContent>
    </Dialog>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (!name) return;
    if (!accountId || !user) {
      toast.error(t('customFields.toasts.notLinked'));
      return;
    }
    if (isDuplicate(name)) {
      toast.error(t('customFields.toasts.duplicate', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('customFields.toasts.createFailed'));
      return;
    }
    toast.success(t('customFields.toasts.createSuccess', { name }));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('customFields.toasts.renameFailed'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        t('customFields.deleteConfirm', { name: field.field_name })
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('customFields.toasts.deleteFailed'));
      return;
    }
    toast.success(t('customFields.toasts.deleteSuccess', { name: field.field_name }));
    await fetchFields();
  }
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('customFields.renameAria', { name: field.field_name })}
      ...
      <Button
        ...
        title={t('customFields.deleteFieldTitle')}
